...
function findReflectionCatalog(startDir: string): string | undefined {
  // Walk up from startDir for project-local override
  let dir = startDir
  for (;;) {
  ...
  // Fall back to global ~/.argon/reflection.json generated by CLI install
  const global = path.join(process.env.HOME ?? "~", ".argon", "reflection.json")
  return fs.existsSync(global) ? global : undefined
}
